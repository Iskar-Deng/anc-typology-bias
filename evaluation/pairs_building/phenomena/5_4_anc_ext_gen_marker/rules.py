...
# evaluation/pairs_building/phenomena/5_4_anc_ext_gen_marker/rules.py

"""
Define perturbation rules for 5.4 ANC external genitive marker.

Usage:
python -m evaluation.pairs_building.apply_perturbation \
  --phenomenon evaluation/pairs_building/phenomena/5_4_anc_ext_gen_marker \
...
def foil_marker_for_gen(row: Dict[str, Any] | None, source_index: int) -> tuple[str, str]:
    if alternating_noun_marker(row, source_index) == "ca":
...
def perturb(
    ...
    context, skip = prepare_anc_external_marker_context(
        ...
        good_marker,
        "gen",
        marker_mismatch_reason="anc_external_gen_marker_does_not_match_expected_ge",
    )
